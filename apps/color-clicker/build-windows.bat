...
taskkill /f /im color-clicker.exe 2>nul
del /f /q dist\color-clicker.exe 2>nul

echo ==^> Building .exe...
uv run pyinstaller color-clicker-windows-x64.spec --clean --noconfirm
if errorlevel 1 goto :error

echo.
echo Done: dist\color-clicker.exe
goto :end
