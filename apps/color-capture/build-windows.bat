...
taskkill /f /im color-capture.exe 2>nul
del /f /q dist\color-capture.exe 2>nul

echo ==^> Building .exe...
uv run pyinstaller color-capture-windows-x64.spec --clean --noconfirm
if errorlevel 1 goto :error

echo.
echo Done: dist\color-capture.exe
goto :end
