...
taskkill /f /im auto-capture-debug.exe 2>nul
del /f /q dist\auto-capture-debug.exe 2>nul

echo =^=> Building debug .exe (with console)...
uv run pyinstaller auto-capture-windows-debug.spec --clean --noconfirm
if errorlevel 1 goto :error

echo.
echo Done: dist\auto-capture-debug.exe  (콘솔 창이 함께 뜨는 디버그용 빌드)
goto :end
